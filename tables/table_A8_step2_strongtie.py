...
def to_dest(users, districts_analyze, if_fix):
    column_name = ['user', 'if_move', 'd', 'l', 's', 'strong5', 'strong12',
                   'date', 't',
                   'home', 'dest']
    ...
    for d_this in districts_analyze:
        ...
        if if_fix:
            ...
            column_new_name = {'s_' + str(d_this): 's',
                               'l_' + str(d_this): 'l',
                               'd_' + str(d_this): 'd',
                               'strong5_' + str(d_this): 'strong5',
                               'strong12_' + str(d_this): 'strong12'}
            column_select = ['user', 'if_move',
                             'd_' + str(d_this),
                             'l_' + str(d_this),
                             's_' + str(d_this),
                             'strong5_' + str(d_this),
                             'strong12_' + str(d_this),
                             'date', 't',
                             'home', 'dest']
...
if __name__ == '__main__':
    ...
    for i in range(loop_num):
        ...
        strongtie = pd.read_csv('data/' + t2_str + '_user_result_strongtie.csv')
        strongtie = strongtie.drop(columns=['home', 'date'])
    ...
    for v in ['d', 'l', 's', 'strong5', 'strong12']:
        for i in dist:
            users[v + '_' + str(i)] = users[v + '_' + str(i)].fillna(value=0)

    if_fix = True
    column_select = ['user', 'if_move',
                     'd', 'l', 's', 'strong5', 'strong12',
                     't', 'date',
                     'home', 'dest']
    # destination
    ...
    column_select.remove('strong5')
    column_select.remove('strong12')
    home = b[column_select]

    # merge home and destination (for pull and push regression)
    dest.columns = ['user', 'if_move', 'd_dest', 'l_dest', 's_dest', 'strong5', 'strong12',
    ...
    dest_home.to_csv('data/dest_home_d_s_l_strongtie.csv', index=None)
