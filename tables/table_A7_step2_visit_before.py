...
def to_dest(users, districts_analyze, if_fix):
    column_name = ['user', 'if_move', 'd', 'l', 's', 'if_visit', 'if_visit_night',
                   'date', 't',
                   'home', 'dest']
    ...
    for d_this in districts_analyze:
        ...
        if if_fix:
            ...
            column_new_name = {'s_' + str(d_this): 's',
                               'l_' + str(d_this): 'l',
                               'd_' + str(d_this): 'd',
                               'if_visit_' + str(d_this): 'if_visit',
                               'if_visit_night_' + str(d_this): 'if_visit_night'}
            column_select = ['user', 'if_move',
                             'd_' + str(d_this),
                             'l_' + str(d_this),
                             's_' + str(d_this),
                             'if_visit_' + str(d_this),
                             'if_visit_night_' + str(d_this),
                             'date', 't',
                             'home', 'dest']
...
if __name__ == '__main__':
    ...
    for i in range(loop_num):
        ...
        if_visit = pd.read_csv('data/' + network_date + '_migrant_if_visit_before.csv')
        users_2 = users_2.merge(if_visit, left_on='user', right_on='id', how='left')
        if i == 0:
            users = users_2
        else:
    ...
    for v in ['d', 'l', 's', 'if_visit', 'if_visit_night']:
        for i in dist:
            users[v + '_' + str(i)] = users[v + '_' + str(i)].fillna(value=0)

    if_fix = True
    column_select = ['user', 'if_move',
                     'd', 'l', 's', 'if_visit', 'if_visit_night',
                     't', 'date',
                     'home', 'dest']
    # destination
    ...
    column_select.remove('if_visit')
    column_select.remove('if_visit_night')
    home = b[column_select]

    # merge home and destination (for pull and push regression)
    dest.columns = ['user', 'if_move', 'd_dest', 'l_dest', 's_dest', 'if_visit', 'if_visit_night',
    ...
    dest_home.to_csv('data/dest_home_d_s_l_visit_before.csv', index=None)
