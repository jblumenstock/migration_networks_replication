def get_accuracy(p):
    pi_home_i_this, pi_dest_i_this, pi_home_c_this, pi_dest_c_this, \
        ...
        lambda_home_this, tao_this, rho_this, t_this, diffusion_p_this = p 
    ...
    all_['move_result'] =\
        all_.apply(lambda x: move_or_not(x['u_diff'], tao_this), axis=1)
...
tao_idx = [[] for i in range(len(tao))]
            ...
            for z4, alpha_home_this in enumerate(alpha_home):
                for z5, alpha_dest_this in enumerate(alpha_dest):
                    for z6, lambda_home_this in enumerate(lambda_home):
                        for z7, tao_this in enumerate(tao):
